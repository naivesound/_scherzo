...
var Slider = {
  ...
  width: 0,
  dots: [],
  oncreate: (vnode) => { m.redraw(); },
  onupdate: (vnode) => {
    Slider.updateSize(vnode);
  },
  view: (vnode) => {
    return m('div', {
      onclick: (e) => vnode.attrs.onprogresschange((e.clientX - vnode.state.x) / vnode.state.width),
      style: {
	display: 'flex',
	flex: 1,
	minWidth: '200px',
...
var Knob = {
  ...
  dots: [],
  view : (c) => {
    return m('.knob-wrapper',
	     m('.knob-icon',
	       m('i.material-icons',
		 {style: {transform: (c.attrs.rotate ? 'rotate(-90deg)' : '')}}, c.attrs.icon)),
	     m('.knob-button.knob-minus', {onclick: () => {c.state.value -= 0.05; }},
	       m('i.material-icons', 'remove')),
	     m(Slider, {
	       value: c.state.value,
	       onprogresschange: (value) => c.state.value = value,
	     }),
	     m('.knob-button.knob-minus', {onclick: () => {c.state.value += 0.05; }},
...
var SliderLayout = {
  view: (vnode) => {
    return m('div', {style: {display: 'flex', flexWrap: 'wrap'}},
	     m('div', {style: {display: 'flex', flex: 1, flexDirection: 'column'}},
	       m(Knob, {icon: 'volume_up', oninput : midiBind(vnode.attrs.scherzo, 0x07)}),
	       m(Knob, {icon: 'timelapse', oninput : midiBind(vnode.attrs.scherzo, 0x0d)})
	      ),
	     m('div', {style: {display: 'flex', flex: 1, flexDirection: 'column'}},
	       m(Knob, {icon: 'repeat', oninput : midiBind(vnode.attrs.scherzo, 0x0c)}),
	       m(Knob, {
		 icon: 'filter_list',
		 rotate: true,
		 oninput : midiBind(vnode.attrs.scherzo, 0x09),
	       })));
  }
};
